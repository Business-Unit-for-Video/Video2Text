...
OUTPUT_ROOT = Path(OUTPUT_ROOT_RAW) if OUTPUT_ROOT_RAW else Path("destinations")
...
def transcribe_audio(model: WhisperModel, audio_path: Path, audio_duration: float) -> Dict:
    ...
    if TRANSCRIBE_CHUNKED:
        return transcribe_audio_chunked(
            ...
            mmss_mmm, TRANSCRIBE_CHUNK_SECONDS,
        )
    return transcribe_audio_whole(model, audio_path, audio_duration, kwargs, mmss_mmm)
...
def git_commit_and_push(message: str):
    git_run(["git", "add", str(OUTPUT_ROOT), "state_collections"], check=False)
